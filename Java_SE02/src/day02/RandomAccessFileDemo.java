...
public class RandomAccessFileDemo {
	public static void main(String[] args){
		...
		try{
			 ...
			 * max>>>8   00000000 00000000 01111111 11111111 11111111 11111111
			 */
			raf.write(max>>24);//写int值的最高的8位
			raf.write(max>>16);
			raf.write(max>>8);
			raf.write(max);
			/**
			 ...
			 * 写一个字符串 按照GBK编码
			 */
			String str = "石孟男大笨蛋lalal！";
			byte[] data = str.getBytes("GBK");
			/**
			 * write(byte[] data)
			 * 一次性将一个字节数组中的数据全部写出
			 * 
			 * 通常我们在写字符串的时候，不是直接把字符串转换的字节先写出去
			 * 而是要先写一个整数，这个整数表示后面我们将要写的字符串的总字节量，
			 * 这样便于读取，否则我们在读取的时候无法确定要读取多少个字节才能
			 * 完整的读取所有字符串。
			 */
